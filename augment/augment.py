...
parser = argparse.ArgumentParser(
    description="Augmentation of an audio dataset applyint time streching, pitch shifting and additive noise. Argument: dataset name"
)
parser.add_argument(
    "-d",
    "--dataset",
    type=str,
    required=True,
    help="Dataset name (folder)",
    choices=["MAVD", "UrbanSound8K", "GRN"],
)
parser.add_argument(
    "-p",
    "--dataset_path",
...
metadata_augmented_path = os.path.join(
    data_path, "metadata/" + dataset + "_augmented.csv"
)
...
if dataset == "MAVD":
    ...
    metadata = metadata_ori.loc[
        lambda metadata: metadata["Fold"] == "train"
    ]  ##IF MAVD IT APPLYS THE AUGMENTATION ONLY ON THE TRAIN SET
else:
...
for rate in rates:
    ...
    for index, row in metadata.iterrows():
        if dataset == "UrbanSound8K":
            curr_file_path = (
                audio_path + "/fold" + str(row["fold"]) + "/" + row["slice_file_name"]
            )
            curr_rate_path = (
                augmented_path
                + "/fold"
            ...
            curr_file_path = (
                audio_path + "/" + row["Fold"] + "/" + row["File"].split("/")[-1]
            )
            curr_rate_path = (
                augmented_path + "/" + row["Fold"] + "/speed_" + str(int(rate * 100))
            )
...
for tone_step in tone_steps:
    ...
    for index, row in metadata.iterrows():
        if dataset == "UrbanSound8K":
            ...
            curr_ps_path = (
                augmented_path + "/fold" + str(row["fold"]) + "/pitch_" + str(tone_step)
            )
            ...
            curr_ps_path = (
                augmented_path + "/" + row["Fold"] + "/pitch_" + str(tone_step)
            )
    ...
    new_meta = pd.DataFrame(
        {
            ...
            "fold": aug_folds,
            "class": aug_classes,
            "class_id": aug_class_id,
            "augment": aug_augmentations,
        }
    )
