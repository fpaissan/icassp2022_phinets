...
parser = argparse.ArgumentParser(
    description="Extract features from an audio dataset. Argument: dataset name boolean augmented features or not"
)
parser.add_argument(
    "-d",
    "--dataset",
...
audio_path = os.path.abspath(dataset_folder + "/audio")
metadata_path = os.path.abspath(
    dataset_folder + "/metadata/" + dataset + "_augmented.csv"
)
    ...
    if dataset == "UrbanSound8K":
        if str(row["augment"]) == "none":
            file_path = os.path.join(
                audio_path, "fold" + str(row["fold"]), str(row["file"])
            )
        else:
            file_path = os.path.join(
                audio_path + "_augmented",
                "fold" + str(row["fold"]),
                str(row["augment"]),
                str(row["file"]),
            )
