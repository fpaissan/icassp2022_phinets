...
def main(config_dir, dataset, dataset_dir, augmentation):
    print(config_dir, dataset, augmentation)
    ...
    for c in confs:
        ...
        if dataset == "UrbanSound8K" or dataset == "GRN":
            ...
            for testfold_index in range(1):
                ...
                valid_acc = trainer.validate(
                    ckpt_path="best", datamodule=data_module, verbose=0
                )
                test_acc.append(
                    trainer.test(ckpt_path="best", datamodule=data_module, verbose=0)[
                        0
                    ]["test/acc"]
                )
                ...
                fold_perf.append(
                    {
                        "test_id": testfold_index,
                        "acc test": test_acc[0],
                        "acc valid": valid_acc[0],
                    }
                )
            print(
            ...
            valid_acc = trainer.validate(
                ckpt_path="best", datamodule=data_module, verbose=1
            )
